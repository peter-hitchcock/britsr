...
Only one trial is less than 200 ms  
```{r}
bx_dfs %>% filter(rt < 200)
```
...
8/2 - Following reviewer request, prep deIDed data to make publicly available by putting in `Public_Data`   

```{r}
qairres <- read.csv("./../../data/raw_files/justDASSandFFMQ_with_imputes_replaced_with_999.csv")
#write.csv(qairres, "./Public_Data/justDASSandFFMQ_with_imputes_replaced_with_999.csv")
```
```{r}
bx_df <- read.csv("./../../data/cleaned_files/s_bdf.csv")
#write.csv(bx_df, "./Public_Data/s_bdf.csv")
```


```{r}
extra_df_full <- read.csv("./../../data/raw_files/tx_cond+ffmq+maas_from_KW_6.24.2020.csv")
#write.csv(extra_df_full %>% select(ID, Treatment), "./Public_Data/tx_assignments.csv")
```
